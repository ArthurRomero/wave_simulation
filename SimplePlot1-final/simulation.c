...
int r =41;// length of the array (ReT and ImT).
...
double (*ReTgenerator(double ReT[][2]))[2];

double (*ImTgenerator(double ReT[][2]))[2];
...
int main( )
{
    ...
    gp0( zstart + 1*zres,r0,el0,w0);
    
    
    
    
    // gp1(0.00266667,r1,el1,w1);
...
double (*ImTgenerator(double ImT[][2]))[2]
{
    double eta = width/d;
    double vel = pow(2*energy*e_charge/e_mass,1/2);
    double alpha = wedgeangle*pi/180;
...
void gp0(double z,double r0,double el0, double w0)
{
    ...
    for(int i=0; i<rows; i++)
    {
        ...
        a[i][1]=exp(-(pi*jj));
        printf("the value of a[i][1] is %f\n",a[i][1]);
...
void gp1(double z12,double r1,double el1, double w1)
{
    double coef;
    ...
    for (int i=0; i<300; i++) {
        for (int n=-lim; n<=lim; n++) {
            ...
            for (int m=-lim; m<=lim; m++) {
                ...
                {
                    coef =  ReT[x2pnts((double *)ReT, r, n)][1]*ReT[x2pnts((double *)ReT, r, m)][1]+ImT[x2pnts((double *)ImT, r, n)][1]*ImT[x2pnts((double *)ImT, r, m)][1];//
                    //printf("the value of eta1 is: %f\n",eta1);
            ...
            for (int m2=-lim; m2<=lim; m2++) {
                for (int n1=-lim; n1<=lim; n1++) {
                    for (int n2=-lim; n2<=lim; n2++) {
                        ...
                        a = (x2pnts((double *)ReT, r, m1));
                        b = (x2pnts((double *)ReT, r, m2));
                        c = (x2pnts((double *)ReT, r, n1));
                        d5 = (x2pnts((double *)ReT, r, n2));
