    ...
    for (int i=0; i<rows; i++) {
        for (int n=-lim; n<=lim; n++)
        {
            ...
            for (int m=-lim; m<=lim; m++)
            {
                ...
                {
                    coef =  ReT[n + ((rowsT-1)/2)]*ReT[m + ((rowsT-1)/2)]+ImT[n + ((rowsT-1)/2)]*ImT[(m + ((rowsT-1)/2))];//n+(rowsT-1)/2 represents the position of the element n in the array ReT. if rowsT is 41 and n is -5 for example, the position would be 15.
               
                }
                
                
                coef = coef*exp(-pi*(dn*(lambda*z12/(pow(d*el2,2)))));
               
                if (isfinite(coef)==0)// added isfinite macro in order to avoid inf values
                {
                    coef=0;
                }
